...
def process_interactive_mode(frames_dir, output_dir, limit=None, compute_background=False, save_interactive_result=False, model_size="base", overwrite=False):
    ...
    while i < len(frame_paths):
        try:
            ...
            mask_file = os.path.join(frame_output_dir, "mask.npy")
            label_file = os.path.join(frame_output_dir, "label.pkl")
            
            if not overwrite and (os.path.exists(mask_file) or os.path.exists(label_file)):
...
def process_sampled_frames(frames_dir, output_dir, limit=None, model_checkpoint="facebook/sam2-hiera-base-plus", overwrite=False):
    ...
    for frame_path in tqdm(frame_paths):
        try:
            ...
            # Check if this frame has already been processed
            mask_file = os.path.join(frame_output_dir, "masks.npy")
            if not overwrite and os.path.exists(mask_file):
                logger.info(f"Skipping {frame_path} as it has already been processed")
            ...
            np.save(os.path.join(frame_output_dir, "masks.npy"), masks)
            np.save(os.path.join(frame_output_dir, "scores.npy"), scores)
            np.save(os.path.join(frame_output_dir, "projected_waypoints.npy"), projected_waypoints)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--frames_dir", default="out/sampled_frames/subset_1", help="Directory containing sampled frames")
